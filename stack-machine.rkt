...
(define (STORE_FAST stack-machine var_i)
  ...
  (pop-exec-stack (update-stack-machine 'CO-VARNAMES (hash-set (get-varnames stack-machine) var_i (top (get-stack stack-machine))) stack-machine))
)
...
(define (JUMP_ABSOLUTE stack-machine target)
  ;(pretty-print "JUMP_ABSOLUTE called")
...
(define (GET_ITER stack-machine uselessParam)
  ;(pretty-print (get-stack stack-machine))
  stack-machine
)

(define (FOR_ITER stack-machine delta)
  ;(pretty-print stack-machine)
  
  (if (null? (top (get-stack stack-machine)))
      (update-stack-machine 'INSTRUCTION-COUNTER (+ (get-IC stack-machine) (/ (+ delta 2) 2)) (pop-exec-stack stack-machine))
      (push-exec-stack (car (top (get-stack stack-machine))) (push-exec-stack (cdr (top (get-stack stack-machine))) (pop-exec-stack stack-machine)))
  )
)

(define (POP_TOP stack-machine uselessParam)
  ;(pretty-print "POP_TOP called")
  (pop-exec-stack stack-machine)
)

(define (CALL_FUNCTION stack-machine argc)
  ;(pretty-print "CALL_FUNCTION called")
  (let iter ((i 0) (args '()) (newStackMachine stack-machine))
    (if (equal? i argc)
        (push-exec-stack (apply (get-function (top (get-stack newStackMachine))) args) (pop-exec-stack newStackMachine))
        (iter (+ i 1) (append (list (top (get-stack newStackMachine))) args) (pop-exec-stack newStackMachine))
    )
  )
)

(define (SETUP_LOOP stack-machine uselessParam)
  ;(pretty-print "SETUP_LOOP skipped")
  stack-machine
)

(define (POP_BLOCK stack-machine uselessParam)
  ;(pretty-print "POP_BLOCK skipped")
  stack-machine
)

(define (RETURN_VALUE stack-machine uselessParam)
